$.fn.wordlist = function(){
	return this.each(function(){
		...
		var $field = $(this), // textarea
			...
			$input = $('<input type="text" class="js-wordlist-input" placeholder="Dán nhãn cho ghi chú..."/>'), // input
		...
		$.each(words, function(i, word){
			...
			$('<span class="wordTag">'+word.trim()+'</span>').appendTo($mask);
		});
		...
		$input.on('keypress', function(e) {
			...
			if (word != '') {
				...
				if (e.which == 13 || e.which == 44) {
					...
					$('<span class="wordTag">'+word.trim()+'</span>').insertBefore($input);
					// Update the contents of the textarea
					$field.val(words.join(','));
